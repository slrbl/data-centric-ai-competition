...
if __name__ == "__main__":
    ...
    for label in labels:
        correct = 0.
        nbr=0
        image_files = os.listdir('label_book/'+label+'/')
        print ('CLASS:{}'.format(label))
        for image_file in image_files:
            if 'png' in image_file:
                nbr+=1
                all_nbr+=1
                pred=single_predict('label_book/'+label+'/'+image_file)
                if pred == label:
                    correct+=1
                    all_correct+=1
                    print('GOOD:'+'label_book/'+label+'/'+image_file)
                else:
                    print('pred:')
                    print(pred)
                    print('label:')
                    print(label)
                    print('BAD:'+'label_book/'+label+'/'+image_file)

        acc = correct/nbr
        print ('Class accuracy:{}'.format(acc))
        print('=============')
    print((all_correct/all_nbr))
